const { findProduct, totalInventory, hasPostgres } = require('./store');
const { getVariantAvailable } = require('./postgres');
const { variantPrice } = require('./pricing');
...
// Stock real desde inventory_levels cuando hay Postgres; espejo del catálogo como respaldo.
async function resolveAvailableStock(product, variant) {
  if (hasPostgres()) {
    const vid = variant ? variant.id : (product.variants && product.variants[0] && product.variants[0].id);
    if (vid) {
      const row = await getVariantAvailable(product.id, vid);
      if (row != null) return Number(row.available) || 0;
    }
  }
  return variant
 ...
 */
async function buildStripeLineItems(catalog, items) {
  const stripeLines = [];
  const orderLines = [];
  let totalChf = 0;
  ...
  for (const item of items) {
    ...
    const stock = await resolveAvailableStock(product, variant);
    if (stock < qty) {
    ...
    const baseCents = Math.round(variantPrice(product, variant)) * 100;
