...
// — COLECCIONES -------------------------------------------------------------
// Devuelve Map<productId, Set<collectionId>> para evaluar discount_conditions
// de tipo `collection` en checkout.
async function getCollectionIdsForProducts(productIds) {
  const db = getSql();
  const map = new Map();
  ...
  try {
    const rows = await db`
      ...
      WHERE product_id IN ${db(ids)}
    `;
    for (const id of ids) map.set(id, new Set());
    for (const r of rows) {
      if (!map.has(r.product_id)) map.set(r.product_id, new Set());
      map.get(r.product_id).add(r.collection_id);
    }
  } catch (err) {
    console.warn('[postgres] getCollectionIdsForProducts:', err.message);
  }
  return map;
}
